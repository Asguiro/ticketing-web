...
- [`API_FRONTEND.md`](./API_FRONTEND.md) — guide d'intégration complet (auth, endpoints, WebSocket)
- [`FRONTEND_TYPES.ts`](./FRONTEND_TYPES.ts) — enums et interfaces TypeScript à recopier
- [`RBAC_MATRIX.md`](./RBAC_MATRIX.md) — permissions par rôle
- Swagger live : `{API_URL}/api/docs`
...
- `action` → `POST /api/v1/auth/login`, stocke tokens (access en mémoire serveur,
  refresh en cookie httpOnly sur le domaine frontend — cf. `API_FRONTEND.md` §2)
...
app/types/api.ts                        // recopié depuis docs/FRONTEND_TYPES.ts
```

---

## Mapping API par page (référence rapide)

| Page | Endpoints appelés |
|------|-------------------|
| LoginPage | `POST /auth/login` |
| DashboardPage | `GET /dashboard/stats` |
| TicketsListPage | `GET /tickets` (+ `GET /users?role=AGENT` pour filtre admin) |
| NewTicketPage | `POST /tickets` |
| TicketDetailPage | `GET /tickets/:id`, `GET /tickets/:id/messages`, `GET /tickets/:id/history` |
| TicketDetailPage (actions) | `POST /tickets/:id/messages`, `PATCH /tickets/:id/status`, `PATCH /tickets/:id/assign` |
| UsersListPage | `GET /users`, `POST /users`, `PATCH /users/:id`, `DELETE /users/:id` |
| RootLayout (logout) | `POST /auth/logout` |

**WebSocket (TicketDetailPage)** : namespace `/chat`, events `joinTicket`, `message:created`, `ticket:reopened`.
Voir `API_FRONTEND.md` §6.
